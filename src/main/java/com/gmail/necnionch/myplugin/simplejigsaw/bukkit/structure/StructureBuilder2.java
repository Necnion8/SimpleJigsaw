...
public class StructureBuilder2 {
    ...
    public StructureBuilder2(StructureConfig.Schematics schematics, int maxSize, Map<String, List<JigsawPart>> partsOfPool) {
    ...
    public int startBuild(Random random, Extent extent, int x, int y, int z, int angle, Map<String, String> bottomFillBlockOfBiomeKey) throws WorldEditException {
        WorldEditBuild build = createBuild(random, extent, x, y, z, angle, bottomFillBlockOfBiomeKey);
        return build.start();
    }

    public WorldEditBuild createBuild(Random random, Extent extent, int x, int y, int z, int angle, Map<String, String> bottomFillBlockOfBiomeKey) {
        List<Operation> operations = Lists.newArrayList();
        int parts = build(random, extent, BlockVector3.at(x, y, z), angle, operations);

        WorldEditBuild build = new WorldEditBuild(null, null, operations, parts);

        // todo: impl: fill bottom
...
//        }
    ...
    private int build(Random random, Extent extent, BlockVector3 position, int angle, List<Operation> operations) {
        if (firstPart == null)
...
//                .build()
//        );
        operations.add(new WrapperPasteBuilder(clipboardHolder, extent, clipboard.getOrigin())
                .to(position)
                .maskSource(createNonReplaceMaskStructureVoid(clipboard))
                .build());
        ...
        return 1 + buildJigsawConnectors(random, firstPart, extent, position, angle, 1, operations);
    }
...
//                .build()
//        );
        connect.operations.add(new WrapperPasteBuilder(clipboardHolder, connect.getExtent(), to.getOriginalLocation())
                .to(position)
                .maskSource(createNonReplaceMaskStructureVoid(clipboard))
                .build());
...
//            showParticle(pos, connect.getWorld(), Color.BLUE);
    ...
    private int buildJigsawConnectors(Random random, JigsawPart part, Extent extent, BlockVector3 position, int angle, int size, List<Operation> operations) {
        // 含まれるConnectorを探し、次のパーツのために座標をもとめる
        int results = 0;
...
//            showParticle(pos, world, Color.LIME);
            ...
            results += expandJigsawPart(new ConnectInstance(random, conn, extent, pos, ori, size + 1, operations));
//            break;
        }
        return results;
...
//        }
        ...
        public ConnectInstance(ConnectInstance connectInstance, Random random, JigsawConnector connector, BlockVector3 position, JigsawConnector.Orientation orientation) {
            ...
            this.extent = connectInstance.extent;
        }

        public ConnectInstance(Random random, JigsawConnector connector, Extent extent, BlockVector3 position, JigsawConnector.Orientation orientation, int size, List<Operation> operations) {
//            this.world = world;
            this.random = random;
            this.connector = connector;
            this.position = position;
